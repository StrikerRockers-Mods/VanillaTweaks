...
public class HopsEnchantment extends Enchantment {
    ...
    public int getMaxLevel() {
        return VanillaTweaksFabric.config.enchanting.enableHops ? 3 : 0;
    }

    @Override
    public boolean canEnchant(ItemStack stack) {
        return stack.getItem() instanceof ArmorItem armorItem && armorItem.getSlot().equals(EquipmentSlot.FEET) &&
                VanillaTweaksFabric.config.enchanting.enableHops;
    }

    @Override
    public boolean isDiscoverable() {
        return VanillaTweaksFabric.config.enchanting.enableHops;
    }


    @Override
    public boolean isTreasureOnly() {
        return VanillaTweaksFabric.config.enchanting.hopsTreasureOnly;
    }
